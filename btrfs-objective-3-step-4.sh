...
START () {
	...
	Uname='btrfs-objective-3-step-4'
...
CHECKER () {
	...
	do
   	...
   	# Check that Btrfs size has been changed to given size
    	ssh root@$IP_to_SSH 'btrfs filesystem show /var/data | grep sdc | tr -s ' ' | cut -d' ' -f4 | grep 9 && btrfs filesystem show /var/data | grep sdd | tr -s ' ' | cut -d' ' -f4 | grep 9 && btrfs filesystem show /var/data | grep sde | tr -s ' ' | cut -d' ' -f4 | grep 9'

   	# Run objectiveschecks.py and update VirtualTa with correct value
    	if [ $? -eq 0 ]; then

        	echo -e "\n Success: Size is set correctly on all disks! Date: `date`\n" && touch $CheckFile
        	$DIR/objectivechecks.py $Uname True || echo -e "\n Failed to run $DIR/objectiveschecks.py! Date: `date`" >&2 && exit 1
        	exit 0

    	else

        	echo -e "Failed: Size is not set correctly on all or one disk! Date: `date`\n" >&2
        	sleep $Sleep
