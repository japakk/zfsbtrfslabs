#!/bin/bash
# Script for checking the deletion of a Btrfs RAID1
...
START () {
	...
	Uname='btrfs-objective-4-step-5'
...
CHECKER () {
	...
	do
   	...
   	# Check that Btrfs RAID1 has been removed
    	ssh root@$IP_to_SSH 'btrfs filesystem df /var/raid1'

   	# Run objectiveschecks.py and update VirtualTa with correct value
    	if [ $? -eq 1 ]; then

        	echo -e "\n Success: Btrfs RAID1 has been removed! Date: `date`\n" && touch $CheckFile
        	$DIR/objectivechecks.py $Uname True || echo -e "\n Failed to run $DIR/objectiveschecks.py! Date: `date`" >&2 && exit 1
        	exit 0

    	else

        	echo -e "Failed: Btrfs RAID1 still exists! Date: `date`\n" >&2
        	sleep $Sleep
