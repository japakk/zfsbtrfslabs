#!/bin/bash
# Script for checking if zfsutils-linux has been installed
...
CHECKER () {
	...
	do
   	...
   	# Check if zfsutils-linux has been installed
    	ssh root@$IP_to_SSH 'dpkg -s zfsutils-linux'

   	# Run objectiveschecks.py and update VirtualTa with correct value
    	if [ $? -eq 0 ]; then

        	echo -e "\n Success: Package zfsutils-linux installed! Date: `date`\n" && touch $CheckFile
        	$DIR/objectivechecks.py $Uname True || echo -e "\n Failed to run $DIR/objectiveschecks.py! Date: `date`" >&2 && exit 1
        	exit 0

    	else

        	echo -e "Failed: Package zfsutils-linux has not been installed! Date: `date`\n" >&2
        	sleep $Sleep
